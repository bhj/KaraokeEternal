...
const PlayerQR = ({ height, prefs, invitationToken, queueItem }: PlayerQRProps) => {
  ...
  const url = useMemo(() => {
    // Smart QR: Route through app endpoint for universal join handling
    if (invitationToken && roomId) {
      const joinUrl = new URL(window.location.origin)
      joinUrl.pathname = `/api/rooms/join/${roomId}/${invitationToken}`
      return joinUrl
    }
    ...
    return url
  }, [roomId, prefs.password, invitationToken])
